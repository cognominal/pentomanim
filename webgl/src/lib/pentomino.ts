...
export const PIECE_ORDER: PieceName[] = ['F', 'I', 'L', 'P', 'N', 'T', 'U', 'V', 'W', 'X', 'Y', 'Z'];

export const PIECES: Record<PieceName, Coord[]> = {
...
export function transform(cells: Coord[], rotation: number, flipped: boolean): Coord[] {
  const out: Coord[] = [];
...
export function cellsForPose(name: PieceName, pose: Pose): Coord[] {
  return transform(PIECES[name], pose.rotation, pose.flipped);
}

export function placeAtAnchor(cells: Coord[], anchorRow: number, anchorCol: number): Coord[] {
  return cells.map(([r, c]) => [r + anchorRow, c + anchorCol]);
}

export function inBounds(cells: Coord[]): boolean {
...
export function solutionKey(placements: Placement[]): string {
  return [...placements]
